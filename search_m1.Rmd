...
read_csv("data/m1_contigs.csv") |>
  filter(allocation %in% c("2L", "NALL")) |>
  # The next line is commented out because the known contigs are a good comparison to the unknown BLAST results
  # filter(!(contig %in% karyotype$Chr)) |> 
  select(contig) |>
  write_csv("output/blast/m1_contigs_filtered.csv")
```
...
```{r}
blast.raw <- read_csv("output/blast/blast_7_9m_to_8_9m_with_known_contigs_head.csv")
``` 


```{r}
blast <- read_csv("output/blast/align_7_9m_to_8_9m_survey.csv") |>
  mutate(length = end - start) |>
  group_by(contig) |>
  summarize(length = sum(length)) |>
  distinct() |>
  left_join(
    read_csv("output/blast/align_7_9m_to_8_9m_query.csv") |>
      mutate(length = end - start) |>
      group_by(contig) |>
      summarize(length = sum(length)) |>
      distinct(),
    by = c("contig"),
    suffix = c(".survey", ".query"))
```


```{r}
blast |> 
  arrange(length.query |> desc())
```
