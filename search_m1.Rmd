...
karyotype <- tibble(
  ...
  species = c("ProtoX", replicate(n - 1 , "target")),
  size = replicate(n, 12),
...
candidates <- read_csv("candidates.csv") |>
  ...
  filter(STATUS != "not in all males") |>
  select(contig = tname)
...
karyotype |> 
  ...
  summarize(proto_chromosome, length_mb = sum(End) / 10^6) |>
  distinct()
```
...
```{r}
karyotype2 <- karyotype |> 
  filter(Chr %in% contigs_of_interest) |>
  left_join(m1.alignment |> select(contig, cq.norm), by = c(
    "Chr" = "contig"
  )) |>
  filter(cq.norm < 0.5) |>
  select(-cq.norm)
  #rbind(karyotype |> filter(species == "ProtoX"))

karyotype2
```

### Check CQ for specific contigs
```{r}
to_check = c(
  # Y
  "tig00003381", # -> X
...
### Find M5 Proto X region to BLAST

```{r}
m1_contigs_on_blast_borders <- c(
  # X
  "tig00009705",
  "tig00009613",
  # Y
...
synteny.bugz_m1_to_m5.filtered |>
  filter(Species2 %in% m1_contigs_on_blast_borders) |>
  select(contig = Species2, Start1, End1) |>
  group_by(contig) |>
  summarize(contig, start_m5 = min(Start1), end_m5 = max(End1)) |>
  distinct() |>
  arrange(contig |> match(m1_contigs_on_blast_borders))
```
- X: 7457321 - 14509517
- Y: 11755973 - 15137911
