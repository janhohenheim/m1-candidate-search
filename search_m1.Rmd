...
```{r}
# These have a good amount of distinctly mapped genes
...
### Split by CQ
```{r}
synteny.bugz_m1_to_m5 |>
  distinct() |>
  group_nest(Start1, End1, Species1) |>
  mutate(data = data |> purrr::map(
    \(dat) dat |> 
      group_nest(Species2))) |>
  mutate(ortholog_count = data |> purrr::map_dbl(nrow)) |>
  arrange(ortholog_count |> desc())
```
...
karyotype.template <- synteny.bugz_m1_to_m5 |>
  distinct() |>
  left_join(m1.alignment, by = c(
    "Species2" = "contig"
  )) |>
  select(Start1, End1, Species1, Species2, Start2, End2, cq.norm) |>
  mutate(chrom = if_else(cq.norm < 0.5, "ProtoY", "ProtoX")) |>
  left_join(alignment.m1_male, by = c("Species2" = "contig")) |>
  filter(chrom == "Y")
```

```{r}
n <- 1 + karyotype.template |>
  nrow()

m5_contig_length <- 17343608
karyotype <- tibble(
  Chr = c(bugz, karyotype.template$Species2),
  Start = replicate(n, 1) |> as.numeric(),
  End =  c(m5_contig_length, karyotype.template$length),
  fill =  c("95FF7D", replicate(n - 1, "8AFFBB")),
  species = c("ProtoX", karyotype.template$chrom),
  size = replicate(n, 12),
...
karyotype.template$Species1 <- karyotype.template$Species1 |>
  get_rank(contig_head)
karyotype.template$Species2 <- karyotype.template$Species2 |>
...
karyotype.template <- karyotype.template |>
  mutate(fill = "FF6E63")
```
