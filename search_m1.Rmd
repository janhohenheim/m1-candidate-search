...
### Fill in gaps
Gaps determined by eye

#### Find gaps
```{r}
gaps.pairs <- tibble(
  chromosome = c("Y", "X"),
  left = c("tig00009367", "tig00009311"),
  right = c("tig00009710", "tig00009613")
)
...
synteny.limits <- synteny |>
  ...
  ungroup() |>
  rbind(tibble(
    start = c(1, 1, proto_x_m5.length, proto_x_m5.length),
    end = c(1, 1, proto_x_m5.length, proto_x_m5.length),
    chromosome = c("X", "Y", "X", "Y"),
    contig = c("X start", "Y start", "X end", "Y end")
  )) |>
...
synteny.limits |> 
  ...
  summarize(
    ...
    after = contig) |>
  group_by(chromosome) |>
  mutate(contig = "{chromosome} gap {row_number()}" |> glue())
...
gaps <- gaps.pairs |> 
  left_join(
    synteny |>
      select(contig = Species2, left.end = End1),
    by = c("left" = "contig")) |>
  group_by(left) |>
  summarize(chromosome, left, right, start = max(left.end)) |>
  distinct() |>  
  left_join(
    synteny |>
      select(contig = Species2, right.start = Start1),
    by = c("right" = "contig")) |>
  group_by(right) |>
  summarize(chromosome, left, right, start, end = min(right.start)) |>
  distinct() |>
  ungroup() |>
  arrange(start) |>
  mutate(contig = "gap {row_number()}" |> glue()) |>
  select(contig, left, right, start, end, chromosome)
...
insertion_indices <- gaps |>
  mutate(index = left |> purrr::map_dbl(\(contig) which(contigs_of_interest == contig)[[1]])) |>
  select(contig, index) |>
  arrange(index |> desc())

for (i in insertion_indices |> seq_along()) {
